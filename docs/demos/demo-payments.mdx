...
This demo also supports an on-chain payment on **Solana devnet** using SPL‑USDC.
In that path, the Client transfers USDC directly to the Server’s Solana address
and includes a Memo bound to the signed Payment Request for replay protection. The
Receipt Service verifies the memo, mint, recipient and exact amount before issuing
a receipt.
