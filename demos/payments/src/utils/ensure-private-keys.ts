...
export async function ensureSolanaKeys(
  ...
  const privateKeyBytes = new Uint8Array(Buffer.from(privateKeyBase64, "base64url"));
  ...
  const secretKey64 = new Uint8Array(privateKeyBytes.length + publicKeyBytes.length);
  ...
  await updateEnvFile({ [pubEnv]: publicKey, [secretEnv]: secretKeyJson }, envFilePath);
