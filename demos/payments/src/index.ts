...
import { jwtStringSchema, paymentRequestSchema } from "agentcommercekit/schemas/valibot";
import * as v from "valibot";
import { isAddress } from "viem";
import { chain, chainId, publicClient, SERVER_URL, solana, usdcAddress } from "./constants";
import { ensureNonZeroBalances, ensureSolanaSolBalance } from "./utils/ensure-balances";
...
`);
  ...
  const [clientPrivateKeyHex, serverPrivateKeyHex, ..._rest] = await Promise.all([
...
The Client attempts to access a protected resource on the Server. Since no valid payment receipt is presented, the Server will respond with an HTTP 402 'Payment Required' status. This response includes a cryptographically signed Payment Request (as a JWT), specifying the required amount, currency, and recipient for the payment.`,
  ...
  log(successMessage("Successfully received 402 Payment Required response from Server Agent. 🛑"));
  ...
  const parsedDetails =
    typeof details === "string" ? await parseJwtCredential(details, resolver) : details;
  ...
  log(sectionHeader("✅ Access Protected Resource with Receipt (Client Agent -> Server Agent)"));
  log(
    colors.dim(
      `${colors.bold("Client Agent 👤 -> Server Agent 🖥️")}
...
If the receipt is valid, the Server grants access to the protected resource.`,
  ...
  await waitForEnter("Press Enter to present the receipt to the Server and access the resource...");

  log(colors.dim("🔐 Making authenticated GET request to the Server Agent with the receipt..."));
...
async function performOnChainPayment(
  ...
  log(sectionHeader("💸 Execute Payment (Client Agent -> Payment Service / Blockchain)"));
  log(
    colors.dim(
...
The Client Agent now uses the details from the Payment Request to make the payment. In this demo, the Client transfers the specified amount of USDC to the Server's address on the Base Sepolia testnet. The resulting transaction hash serves as a preliminary proof of payment. This interaction implicitly uses the blockchain as a Settlement Network and the wallet interaction as a form of Payment Service.`,
  ...
  if (paymentOption.currency !== "USDC") {
    throw new Error(errorMessage(`Unsupported currency: ${paymentOption.currency}`));
  }
  ...
  log(successMessage("Payment transaction submitted to the blockchain. 🚀"), "Transaction hash:");
  ...
  log(colors.dim("⏳ Waiting for transaction confirmation (this might take a moment)..."));
...
async function performSolanaPayment(
  ...
  const keyBytes = new Uint8Array(JSON.parse(clientSolKeys.secretKeyJson) as number[]);
  ...
  const txMessage = pipe(
    ...
    (m) =>
      appendTransactionMessageInstructions(
        [
          ...(maybeCreateRecipientAtaInstruction ? [maybeCreateRecipientAtaInstruction] : []),
          transferIx,
        ],
        m,
      ),
  ...
  const base58Signature = await rpc.sendTransaction(wireTx, { encoding: "base64" }).send();
...
async function performStripePayment(
  ...
  log(sectionHeader("💸 Execute Payment (Client Agent -> Payment Service / Stripe)"));
  log(
    colors.dim(
...
`,
  ...
  if (!returnToUrl) {
    throw new Error(errorMessage("Invalid payment URL - missing return_to parameter"));
  }
